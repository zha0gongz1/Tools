...
sleep 1
#Katana
cat $domain-subdoamin.txt | katana -iqp -kf all > $domain-katana.txt
sleep 1
cat $domain-katana.txt $domain-gau.txt | sort | uniq > $domain-links.txt

rm subDomainsBrute.txt NewSubDomainsBrute.txt subfinder.txt oneforall.txt $domain-gau.txt $domain-katana.txt

# Nuclei
echo -e "\033[5m\033[0;36m[+]Start detecting vulns...\033[0m"
nuclei -t /root/nuclei-templates/ -severity critical,high,medium,low -l $domain-subdoamin.txt -bs 30 -c 30 -rl 120 -nc -o $domain-nuclei-res.txt
